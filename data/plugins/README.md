...
- [`route_optimizer.lua`](https://github.com/ramisotti13-eng/farever-minimap/blob/main/examples/plugins/route_optimizer.lua) — nearby object finder. A heading-up **radar** centred on you shows a coloured dot for every chest / orb / ore / plant from `farever.pois()` within your radius (through walls), plus a big "nearest" readout (distance, up/down, which way to turn) and a proximity ping. An optional, off-by-default **Advanced** toggle plans a multi-stop gathering route (greedy nearest-neighbour + bounded 2-opt) with a live nav arrow and auto-advance. Patterns: world-delta → heading-local radar projection, `imgui.font_scale` for big text, optional `farever.waypoints` pinning.
- [`command_deck.lua`](https://github.com/ramisotti13-eng/farever-minimap/blob/main/examples/plugins/command_deck.lua) — a **5-tab HUD** in one window. **Boss**: a self-learning cast coach that correlates each target cast with your `health_pct` to teach itself which boss skills are dangerous, persisting the threat profile per boss+skill and pre-warning only on the casts that hurt. **Vitals**: only the resources your class uses (max learned by observation), a CAPPED-resource warning, and buffs/debuffs with countdowns. **Drops**: target a mob → its drop table from an embedded FareverDB copy, colour-coded by rarity. **Craft**: search a recipe and tick off ingredients with persistent manual counts (the API exposes no inventory/bank to read), each unmet ingredient annotated with how to obtain it. **Gear**: reads your worn `equipment()` and, per slot, lists higher-rarity items plus their source (craft job/level or which mob drops them, and where). Patterns: event-driven learning, scalar-store table serialization, init-time indices over a static DB, target/equipment-driven lookups.
