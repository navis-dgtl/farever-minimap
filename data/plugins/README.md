...
- [`route_optimizer.lua`](https://github.com/ramisotti13-eng/farever-minimap/blob/main/examples/plugins/route_optimizer.lua) — gathering route optimizer. Filters `farever.pois()` by category / resource sub-type / radius, picks the nearest N nodes, then orders them with greedy nearest-neighbour + a bounded 2-opt pass for a short multi-stop path. Draws the whole route on a schematic map, a live nav arrow to the current stop, and auto-advances (toast + ping) as you reach each one. Patterns: multi-target routing over the POI table, bounding-box map projection, persistent "done" set, optional `farever.waypoints` pinning.
