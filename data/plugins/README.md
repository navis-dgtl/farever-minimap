...
- [`command_deck.lua`](https://github.com/ramisotti13-eng/farever-minimap/blob/main/examples/plugins/command_deck.lua) — a **tabbed HUD** that hosts several tools in one window. The **Boss** tab is a self-learning cast coach: it watches the target's cast bar and correlates each cast with your `health_pct` to teach *itself* which of a boss's skills are dangerous, persisting the threat profile per boss+skill so it pre-warns (sound + toast) only on the casts that actually hurt. The **Vitals** tab shows only the resources your class uses (learning each one's max by observation), flags a CAPPED resource that is wasting regen, and lists buffs/debuffs with countdown bars. Patterns: `cast_start`/`cast_end` event learning, scalar-store table serialization, observed-max tracking, arc drawing via `draw_line` segments.
